...
#
# У production нейронні мережі часто виконують inference для кількох прикладів
# одночасно. Таку групу прикладів називають **батчем** (batch). GPU може
...
#
# Транспонування міняє рядки та стовпці матриці місцями. Воно потрібне тут,
# щоб внутрішні розміри для `matmul(X, W_T)` збігалися.
#
# ```python
